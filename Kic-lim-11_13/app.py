...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_record and check_password_hash(user_record['password_hash'], password):
            ...
            return redirect(url_for('dashboard')) # ✨ [수정] 로그인 후 인사관리로 이동
        else:
...
@login_required
def dashboard():
    """대시보드 페이지를 보여줍니다. (공지사항, 차트, 근무 현황 데이터 포함)"""
    ...
    # 3. [신규] 근무 현황 데이터 (attendance 라우트에서 가져온 임시 로직)
    ...
    for emp in all_employees:
        emp_id = emp['id']
        status = '부재' # 기본값
        if emp_id == '25HR0001': status = '재실'
        elif emp_id == '25DV0002': status = '휴가'
        elif emp_id == '25DV0003': status = '재실'
        elif emp_id == '25HR0005': status = '출장'
        elif emp_id == '25DS0006': status = '외근'
        today_attendance_data[emp_id] = {'status': status} # 간소화
    
    total_employees_count = len(today_attendance_data)
    status_counts = {'재실': 0, '휴가': 0, '외근/출장': 0, '부재': 0}
    
    for emp_data in today_attendance_data.values():
    ...
    return render_template('dashboard.html',
                           ...
                           status_counts=status_counts, # ⬅️ [추가]
...
def hr_management():
    """
    이 라우트는 이제 '목록으로' 버튼 등 기존 링크를 위해 사용되며,
    새로운 기본 페이지인 '사원 명부'로 리디렉션됩니다.
    """
    return redirect(url_for('employee_list'))

@app.route('/hr/list') # ⬅️ '사원 명부'를 위한 새 URL
@login_required 
def employee_list():
    """
    (구 hr_management)
    공지사항과 차트를 제외한 '사원 명부' 페이지만 렌더링합니다.
    """
    
    # --- 1. 출퇴근 버튼 상태 결정 로직 (기존과 동일) ---
    ...
    # ⬅️ [제거] 공지사항 및 차트 데이터 쿼리 제거
    
    conn.close()
    
    # ⬅️ 'employee_list.html'을 렌더링하고 차트/공지 데이터 제외
...
def settings_management():
    ...
    # [수정] 직급 테이블에서 기본급(base_salary)도 함께 조회
...
def edit_position():
    """(신규) 직급명과 기본급을 수정합니다."""
    original_name = request.form['original_pos_name']
    new_name = request.form['new_pos_name'].strip()
    # 폼에서 숫자를 받아옴 (0원일 경우 0으로)
    new_salary = int(request.form.get('new_base_salary', 0))

    try:
        conn = sqlite3.connect('employees.db')
        cursor = conn.cursor()
        # [수정] base_salary 항목도 함께 업데이트
        cursor.execute("UPDATE positions SET name = ?, base_salary = ? WHERE name = ?", 
                       (new_name, new_salary, original_name))
        # [중요] 직원 테이블의 직급명도 모두 갱신
        cursor.execute("UPDATE employees SET position = ? WHERE position = ?", (new_name, original_name))
        conn.commit()
        flash("직급 정보가 성공적으로 수정되었습니다.", "success")
    except sqlite3.IntegrityError:
        flash("이미 존재하는 직급명입니다.", "error")
    finally:
        conn.close()
    return redirect(url_for('settings_management'))

@app.route('/hr/settings/add_deduction', methods=['POST'])
@admin_required
def add_deduction():
    """(신규) 공제 항목을 추가합니다."""
    name = request.form['deduction_name'].strip()
    # 퍼센트(%)로 입력받은 값을 0.0xxx 형태의 비율(REAL)로 변환
    rate = float(request.form.get('deduction_rate', 0)) / 100.0

    if name and rate > 0:
        try:
            conn = sqlite3.connect('employees.db')
            cursor = conn.cursor()
            cursor.execute("INSERT INTO deductions (name, rate) VALUES (?, ?)", (name, rate))
            conn.commit()
            flash(f"'{name}' 항목이 추가되었습니다.", "success")
        except sqlite3.IntegrityError:
            flash("이미 존재하는 항목입니다.", "error")
        finally:
            conn.close()
    else:
        flash("항목명과 공제율(%)을 올바르게 입력하세요.", "error")
    return redirect(url_for('payroll_deductions'))

@app.route('/hr/settings/delete_deduction/<int:deduction_id>', methods=['POST'])
@admin_required
def delete_deduction(deduction_id):
    ...
    try:
        conn = sqlite3.connect('employees.db')
        cursor = conn.cursor()
        cursor.execute("DELETE FROM deductions WHERE id = ?", (deduction_id,))
        conn.commit()
        flash(f"공제 항목이 삭제되었습니다.", "success")
    except Exception as e:
...
@login_required # 로그인이 필요합니다
def payroll():
    """(직원용) 급여 명세서 조회 페이지를 렌더링합니다."""
    
    # (임시) 현재 날짜를 기준으로 12개월치 임시 명세서 데이터를 생성합니다.
    # 나중에 이 부분을 DB에서 조회하도록 변경해야 합니다.
    mock_payslips = []
    today = datetime.now()
    
    for i in range(0, 12):
        # 한 달씩 빼면서 날짜 생성
        pay_date_obj = today - relativedelta(months=i)
        
        # 지급일 (매월 25일로 가정)
        pay_day = pay_date_obj.replace(day=25).strftime('%Y-%m-%d')
        
        # 지급 월 (YYYY년 MM월)
        pay_month = pay_date_obj.strftime('%Y년 %m월')
    ...
    return render_template('payroll.html', payslips=mock_payslips)

@app.route('/payroll/manage')
@admin_required # ⬅️ 관리자 전용
def payroll_manage():
    """(관리자용) 급여 계산기를 렌더링하고 계산을 수행합니다."""
    ...
    # --- 1. 폼 데이터 및 필터 값 가져오기 ---
    pay_month = request.args.get('pay_month', '') # 예: "2025-11"
    name_query = request.args.get('name', '')
    department_query = request.args.get('department', '')
    
    # --- 2. DB에서 직원 + 직급별 기본급 조회 ---
    sql = """
        SELECT e.id, e.name, e.department, e.position, p.base_salary 
        FROM employees e
    ...
    # [수정] 하드코딩된 공제율 삭제
    # DEDUCTION_RATE = 0.0932 (삭제)
    
    # [신규] DB에서 실제 공제율 합계(SUM)를 가져옴
    cursor.execute("SELECT SUM(rate) as total_rate FROM deductions")
    total_rate_row = cursor.fetchone()
    # (공제 항목이 하나도 없으면 0으로)
    TOTAL_DEDUCTION_RATE = total_rate_row['total_rate'] if total_rate_row and total_rate_row['total_rate'] else 0

    if pay_month:
        for emp in employees:
            base_salary = emp['base_salary']
            
            # [수정] DB에서 가져온 공제율 합계(TOTAL_DEDUCTION_RATE)를 사용
            deduction = base_salary * TOTAL_DEDUCTION_RATE
            net_pay = base_salary - deduction
    ...
    # --- 4. 템플릿에 필요한 부서 목록 (필터용) ---
    cursor.execute("SELECT name FROM departments ORDER BY name")
    departments = cursor.fetchall()
    conn.close()
    
    # --- 5. 템플릿 렌더링 ---
    return render_template('payroll_manage.html', 
                           payroll_results=payroll_results, 
                           departments=departments,         
                           pay_month=pay_month,             
                           request=request,
                           # [신규] 계산에 사용된 총 공제율을 템플릿으로 전달 (표시용)
                           total_deduction_rate_percent=TOTAL_DEDUCTION_RATE * 100.0
                           )

@app.route('/payroll/deductions')
@admin_required
def payroll_deductions():
    """(신규) 급여 공제 항목 관리 페이지 (설정에서 이동)"""
    conn = sqlite3.connect('employees.db')
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    # (기존 settings_management에 있던 로직)
    cursor.execute("SELECT * FROM deductions ORDER BY name")
    deductions = cursor.fetchall()
    conn.close()
    return render_template('payroll_deductions.html', deductions=deductions)
